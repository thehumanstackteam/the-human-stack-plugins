...
paraphrase these instructions inline. If someone asks to "analyze calls" in
conversation, invoke `/project-evident:coaching-call-analyzer` or `/run-pipeline`.
...
```
[{ISO 8601 timestamp}] [v1.3.0] [stage-1:phase-a:retrieval] [{Short Name}]
  Status: SUCCESS
...
Task(
  ...
  prompt: "You are analyzing a single coaching call transcript for a Project Evident
    ...
    **When NOT to use the proxy -- leave the field blank or mark as qualitative:**
    ...
    [{ISO 8601 timestamp}] [v1.3.0] [stage-1:phase-b:analyzer] [{Short Name}] [call-{N}]
      Status: SUCCESS
...
1. Log the launch of all Phase B agents to pipeline.log:
   ```
   [{ISO 8601 timestamp}] [v1.3.0] [stage-1:phase-b:launched] [{Short Name}]
...
```
[{ISO 8601 timestamp}] [v1.3.0] [stage-1:complete] [{Short Name}]
  Status: COMPLETE
